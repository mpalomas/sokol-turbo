...
/*
    sokol_app_turbo.h    -- short description

    Project URL: https://github.com/mpalomas/sokol-turbo
...
SOKOL_PRIVATE void _sapp_init_macos(const sapp_desc* desc) {

}

SOKOL_PRIVATE void _sapp_init_win32(const sapp_desc* desc) {
    
}

SOKOL_PRIVATE void _sapp_init_linux(const sapp_desc* desc) {
    
}

SOKOL_API_IMPL void sapp_setup(const sapp_desc* desc) {
#if defined(_WIN32)
    _sapp_init_win32(desc);
#elif defined(__APPLE__) && defined(__MACH__)
    _sapp_init_macos(desc);
#elif defined(__EMSCRIPTEN__)
    
#else
    _sapp_init_linux(desc);
#endif
}
